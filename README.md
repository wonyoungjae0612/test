...
현재 노트북에는 기존 학습 모델이 없습니다. 이 저장소의 실행 가능한 기능은 업로드 UI 데모이며, 로컬 API·Chrome 확장 프로그램·Windows 앱·실제 추론은 아직 구현 전입니다. 72% 예시는 초기 정책(20%/80%)에 따라 UNKNOWN으로 표시합니다. 기존 Vercel 배포 흐름은 웹 화면을 위한 것입니다.
